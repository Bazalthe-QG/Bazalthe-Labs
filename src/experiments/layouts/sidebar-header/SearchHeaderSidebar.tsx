// Search-prominent header with sidebar
...
export default function SearchHeaderSidebar() {
  ...
  return (
    <Frame>
      <div className="h-full flex flex-col">
        <div className="h-8 shrink-0 px-3 flex items-center gap-2" style={nav}>
          <div
            className="flex-1 h-4 rounded-full"
            style={{ background: 'var(--c-border)', opacity: 0.5 }}
          />
          <div className="w-4 h-4 rounded" style={{ background: 'var(--c-border)' }} />
        </div>
          ...
          <div className="w-14 shrink-0 p-1.5 flex flex-col gap-1" style={side}>
            {[1, 2, 3, 4].map((i) => (
              ...
              >
                ...
                />
                <Line w="w-full" opacity={i === 1 ? 0.6 : 0.25} />
              </div>
            ))}
          </div>
