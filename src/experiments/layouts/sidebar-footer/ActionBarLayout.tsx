// Sidebar with persistent action bar footer
  ...
  return (
    <Frame>
      <div className="h-full flex flex-col">
        ...
        <div className="h-7 shrink-0 px-3 flex items-center justify-between" style={footer}>
          <Line w="w-16" opacity={0.5} />
          <div className="flex gap-1.5">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-4 w-10 rounded"
                style={{ background: 'var(--c-border)', opacity: i === 3 ? 0.8 : 0.4 }}
              />
            ))}
          </div>
        </div>
