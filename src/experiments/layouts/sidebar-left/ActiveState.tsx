// Active item indicator styles comparison
function Frame({ children }: { children: React.ReactNode }) {
  return (
    <div className="w-full h-40 overflow-hidden rounded-xl" style={{ background: 'var(--c-bg)' }}>
      {children}
    </div>
  )
}

const Line = ({ w = 'w-full', opacity = 1 }: { w?: string; opacity?: number }) => (
  <div className={`h-1.5 rounded-full ${w}`} style={{ background: 'var(--c-border)', opacity }} />
)

const Dot = () => (
  <div className="w-3 h-3 rounded-sm shrink-0" style={{ background: 'var(--c-border)' }} />
)

export default function ActiveState() {
  const side = { background: 'var(--c-sidebar)', borderRight: '1px solid var(--c-border)' }
  const styles = [
    {
      label: 'text-bg',
      style: {
        background: 'rgba(109,40,217,0.12)',
        color: 'var(--c-accent)',
        borderRadius: 6,
        padding: '2px 4px',
      },
    },
    { label: 'left-bar', style: { borderLeft: '2px solid var(--c-accent)', paddingLeft: 4 } },
    {
      label: 'pill',
      style: { background: 'var(--c-accent)', borderRadius: 9999, padding: '2px 6px' },
    },
    { label: 'underline', style: { borderBottom: '1.5px solid var(--c-accent)' } },
  ]

  return (
    <Frame>
      <div className="h-full flex">
        <div className="w-20 shrink-0 p-2 flex flex-col gap-1" style={side}>
          {styles.map((item, i) => (
            <div key={i} className="flex items-center gap-1 py-0.5" style={item.style}>
              <Dot />
              <Line w="w-full" opacity={i === 0 ? 0.8 : 0.4} />
            </div>
          ))}
        </div>
        <div className="flex-1 p-3 flex flex-col gap-2">
          <Line w="w-3/4" />
          <Line />
        </div>
      </div>
    </Frame>
  )
}
