      ...
      {(['categorize', 'group'] as ViewMode[]).map((m) => (
        <button
          ...
          style={{
            ...
            color: viewMode === m ? 'var(--c-accent)' : 'var(--c-muted)',
...
export default function CategoryHub({ category, onNavigate, viewMode, onViewModeChange }: Props) {
  // Group mode — render all sub-page content
  if (viewMode === 'group') {
    return (
  ...
  // Categorize mode — show sub-page cards
  return (
    <div className="p-10">
      {/* ── Category header ── */}
      <div className="mb-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
        {category.subPages.map((sp, i) => (
          <button
            ...
            className="group flex flex-col gap-2 p-3.5 rounded-xl border text-left transition-all duration-200 hover:shadow-md"
            style={{
            ...
            onMouseEnter={(e) => {
              ...
              ;(e.currentTarget as HTMLButtonElement).style.background = 'rgba(109,40,217,0.03)'
            }}
            onMouseLeave={(e) => {
